...
ij <- bind_rows(
  ...
  plot_data_small_title("pd.merge(x, y, how = 'inner', on = 'num_col')") %>%
  animate_plot()

ij <- animate(ij)
anim_save(here::here("images", "inner-join_pandas.gif"), ij)

ij_g <- inner_join(x, y, by = "id") %>%
  proc_data() %>%
  mutate(.x = .x + 1) %>%
  plot_data_join("pd.merge(x, y, how = 'inner', on = 'num_col')")
