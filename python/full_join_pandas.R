...
fj <- initial_join_dfs %>%
  ...
  plot_data_small_title("pd.merge(x, y, how = 'outer', on = 'num_col')") +
...
fj <- animate(fj)
anim_save(here::here("images", "full-join_pandas.gif"), fj)

fj_g <- full_join(x, y, "id") %>%
  proc_data() %>%
  mutate(.x = .x + 1) %>%
  plot_data_join("pd.merge(x, y, how = 'outer', on = 'num_col')", ylims = ylim(-4.5, -0.5))
