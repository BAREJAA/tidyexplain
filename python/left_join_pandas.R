...
lj <- bind_rows(
  ...
  arrange(value) %>%
  plot_data_small_title("pd.merge(x, y, how = 'left', on = 'num_col')") %>%
  animate_plot()

lj <- animate(lj)
anim_save(here::here("images", "left-join_pandas.gif"), lj)

lj_g <- plot_data_join(lj_joined_dfs, "pd.merge(x, y, how = 'left', on = 'num_col')")
