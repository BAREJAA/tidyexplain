...
sg_data <- bind_rows(
  ...
  mutate(frame = factor(frame, labels = c('pivot_wider()', 'pivot_longer()'))) %>%
...
sg_static <-
  ...
  imap(~ plot_data(.x, .y) +
         ...
         theme(plot.subtitle = element_text(family = "Fira Sans", size = 7, color = "grey50", hjust = 0.5, margin = margin(25)))
  )

save_static_plot(sg_static[[1]], "pivot_wider")
save_static_plot(sg_static[[2]], "pivot_longer")
...
sg_anim <-
  ...
  animate_plot() +
  view_follow() +
  labs(title = "{ifelse(transitioning, next_state, ifelse(grepl('longer', next_state), 'long', 'wide'))}") +
  ease_aes("sine-in-out", x = "exponential-out")

sg_anim <- animate(sg_anim)
anim_save(here::here("images", "pivot_longer_wider.gif"), sg_anim)
